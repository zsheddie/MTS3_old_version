...
class Learn:
    ...
    def train_step(self, train_obs: np.ndarray, train_act: np.ndarray, train_targets: np.ndarray, train_obs_valid: np.ndarray, train_task_valid: np.ndarray, train_task_idx: np.ndarray,
        ...
        """
        self._model.train()
        dataset = TensorDataset(train_obs, train_act, train_targets, train_obs_valid, train_task_valid, train_task_idx)
        ...
        task_id_list = []
        act_vis_list = []
        for batch_idx, (obs, act, targets, obs_valid, task_valid, task_id) in enumerate(loader):
        ...
        act_vis = np.concatenate(act_vis_list, axis=0)
        return avg_loss, avg_metric_nll, avg_metric_rmse, l_vis_prior, l_vis_post, task_labels, act_vis, t.time() - t0

    
    def eval(self, obs: np.ndarray, act: np.ndarray, targets: np.ndarray, obs_valid: np.ndarray, task_valid: np.ndarray, task_idx: np.ndarray,
        ...
        self._model.eval()
        dataset = TensorDataset(obs, act, targets, obs_valid, task_valid, task_idx)
        loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=2)
        ...
        for batch_idx, (obs_batch, act_batch, targets_batch, obs_valid_batch, task_valid_batch, task_id) in enumerate(loader):
            with torch.no_grad():
...
#
        ...
        task_labels = np.concatenate(task_id_list, axis=0)
        act_vis = np.concatenate(act_vis_list, axis=0)
        return avg_loss, avg_metric_nll, avg_metric_rmse, l_vis_prior, l_vis_post, task_labels, act_vis
    ...
    def train(self, train_obs: torch.Tensor, train_act: torch.Tensor,
        ...
        for i in range(self._epochs):
            ...
            train_loss, train_metric_nll, train_metric_rmse, l_vis_prior, l_vis_post, l_labels, act_vis, time = self.train_step(train_obs,
                                                                                                        train_act,
                                                                                                        train_targets,
                                                                                                        train_obs_valid,
                                                                                                        train_task_valid,
            ...
            if val_obs is not None and val_targets is not None and i % val_interval == 0:
                val_loss, val_metric_nll, val_metric_rmse, l_vis_prior_val, l_vis_post_val, l_labels_val, act_vis_val = self.eval(val_obs,
                                                                                                        val_act,
                                                                                                        val_targets,
                                                                                                        val_obs_valid,
                                                                                                        val_task_valid,
