...
class Learn:
    ...
    def __init__(self, model: acRKN, config = None, run=None, log=True, use_cuda_if_available: bool = True):
        """
        ...
        self.vis_dim = self.c.acrkn.latent_obs_dim
        if self._log:
            self._run = run
    ...
    def train_step(self, train_obs: np.ndarray, train_act: np.ndarray, train_targets: np.ndarray,
        ...
        for batch_idx, (obs, act, targets, obs_valid, task_id) in enumerate(loader):
            ...
            out_mean, out_var = self._model(obs_batch,
                                                                                                     act_batch,
                                                                                                     obs_valid_batch)  ##TODO: check if train=True is needed
            ...
            # Clip Gradients
            if self.c.acrkn.clip_gradients:
